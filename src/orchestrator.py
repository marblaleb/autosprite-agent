...
from src.processor import decode_image, remove_background, assemble_spritesheet
...
class Orchestrator:
    ...
    def run(
        ...
        frames = []
        init_b64 = self._image_to_base64(reference_path) if reference_path else None

        for i in range(num_frames):
            if progress_callback:
                progress_callback(i + 1, num_frames)

            prompt = build_prompt(character_desc, animation_type, num_frames, i)
